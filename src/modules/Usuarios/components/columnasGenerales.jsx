...
export const columnasGenerales = (handleDetails, handleEdit, handleToggleStatus, handleDelete) => [
    ...
    // { header: 'Identifiación', accessor: 'identificacion' },
    {
        header: "Contacto",
        accessor: 'contacto',
        Cell: ({ row }) => (
            <div>
                {/* Estiliza el teléfono: alineado a la izquierda y color personalizado */}
                <div style={{ textAlign: 'left' }}>{row.telefono}</div>
    
                {/* Estiliza el correo: alineado a la derecha y color personalizado */}
                <div className="text-xs font-semibold" style={{ textAlign: 'left', color: '#496aa9'}}>{row.correo}</div>
            </div>
        )
    },
